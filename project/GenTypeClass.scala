...
object TypeClass {
  ...
  lazy val cofoldable = TypeClass("Cofoldable", |*->*|->*)
  //lazy val cofoldable = TypeClass("Cofoldable", |*->*|->*, extendsList = Seq(cofoldable1), createSyntax = false)
  //lazy val cofoldable1 = TypeClass("Cofoldable1", |*->*|->*, extendsList = Seq(), createSyntax = false)
    ...
    def extendsToSyntaxListText = kind match {
      ...
      case Kind.|*->*|->* =>
        "extends To" + typeClassName + "Ops0[TC]" + (extendsList match {
          case Seq() => ""
          case es    => es.map(n =>
            "To" + n + "Ops[λ[F[_] => TC[F, S] forSome { type S }]]"
          ).mkString(" with ", " with ", "")
        })
      case _    =>
        extendsList match {
