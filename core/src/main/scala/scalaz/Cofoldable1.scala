...
trait Cofoldable1[F, A] {
  
  def unfoldr1[B](b: B)(f: B => (A, Option[B])): F

  def fromNonEmptyList(l: NonEmptyList[A]): F = {
  ...
  trait Cofoldable1Law {
    ...
    def toNelWeakIdempotence[F[_], A](fa: F[A])(implicit G: Cofoldable1[F[A], A], E: Equal[NonEmptyList[A]], F: Foldable1[F]): Boolean = {
    ...
    def fromNelWeakIdempotence[F[_], A](fa: F[A])(implicit G: Cofoldable1[F[A], A], E: Equal[F[A]], F: Foldable1[F]): Boolean = {
...
object Cofoldable1 {
  @inline def apply[F, A](implicit F: Cofoldable1[F, A]): Cofoldable1[F, A] = F
  //@inline def apply[F[_], A](implicit F: Cofoldable1[F[A], A]): Cofoldable1[F[A], A] = F
}
